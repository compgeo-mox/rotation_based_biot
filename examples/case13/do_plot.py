...
def main():
    ...
    for t_step in np.arange(1, num_time_steps, plot_every):
        ...
        data = np.loadtxt("../case13/pol/" + vtu_file, skiprows=1, delimiter=",")
        data_hyb = np.loadtxt("../case14/pol/" + vtu_file, skiprows=1, delimiter=",")
        ...
        p_num_hyb = data_hyb[::x_every, 0]

        plt.plot(x_num[1:-1], p_num[1:-1] * 1e1, marker="o", linestyle="None", color=color, mfc='none', markersize=10)
        plt.plot(x_num[1:-1], p_num_hyb[1:-1] * 1e1, marker="x", linestyle="None", color=color, markersize=10)
