...
def main():
    ...
    for t_step in np.arange(1, num_time_steps, plot_every):
        ...
        u_num_hyb = data_hyb[::x_every, 2]

        plt.plot(x_num[1:-1], u_num[1:-1] * 1e4, marker="o", linestyle="None", color=color, mfc='none', markersize=10)
        plt.plot(x_num[1:-1], u_num_hyb[1:-1] * 1e4, marker="x", linestyle="None", color=color, markersize=10)
